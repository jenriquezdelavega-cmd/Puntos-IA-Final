...
if [ ! -d "$ROOT_DIR/web" ]; then
  echo "Error: ejecuta este script desde la raiz del repo (carpeta que contiene /web)."
  exit 1
fi

cd "$ROOT_DIR"

echo "[1/4] Escribiendo archivos completos del scanner admin..."
...
if ! rg -n "@yudiel/react-qr-scanner" web/package.json >/dev/null; then
  echo "Falta dependencia @yudiel/react-qr-scanner en web/package.json"
  exit 1
fi
...
echo "[4/4] OK"
echo "Scanner admin restaurado correctamente."

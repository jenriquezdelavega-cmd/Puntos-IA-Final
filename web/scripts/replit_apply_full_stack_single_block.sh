...
if [ ! -d "$ROOT_DIR/web" ]; then
  echo "Error: corre esto desde ~/workspace (raíz del repo)."
  exit 1
fi

cd "$ROOT_DIR"
mkdir -p web/app/admin web/app web/app/pass web/app/api/pass/[customer_id] web/app/api/pass/resolve-token web/app/lib web/app/api/wallet/apple
mkdir -p web/app/api/pass

echo "[1/3] Escribiendo archivos completos..."
...
cat > web/app/api/pass/[customer_id]/route.ts <<'TS'
...
npx eslint app/pass/page.tsx app/api/pass/[customer_id]/route.ts app/api/pass/resolve-token/route.ts app/lib/customer-token.ts app/api/wallet/apple/route.ts

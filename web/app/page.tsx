...
export default function Home() {
  ...
  // ESTADO PARA EL CÓDIGO DE PREMIO
  ...
  const handleRegister = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch('/api/user/register', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ name, phone, password, gender: 'Hombre', birthDate }) }); // Simplificado
  ...
  const handleScan = async (result: string) => {
    ...
    setScanning(false);
    try {
      const res = await fetch('/api/check-in/scan', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ userId: user?.id, code: result }) });
      const data = await res.json();
  ...
  const getPrizeCode = async (tenantId: string, tenantName: string) => {
    if(!confirm(`¿Quieres canjear tus puntos en ${tenantName}?`)) return;
    try {
  ...
  return (
    <div className="min-h-screen bg-gray-100 pb-24">
      ...
      {prizeCode && (
        <div className="fixed inset-0 bg-black/90 z-50 flex flex-col items-center justify-center p-6 animate-fadeIn">
          <div className="bg-white p-8 rounded-3xl text-center w-full max-w-sm relative">
            ...
            <p className="text-gray-500 uppercase text-xs font-bold tracking-widest mb-2">MUESTRA ESTO EN CAJA</p>
            <h2 className="text-2xl font-bold text-blue-600 mb-4">{prizeCode.tenant}</h2>
            <div className="bg-yellow-100 border-2 border-yellow-400 p-6 rounded-2xl mb-4">
               <p className="text-5xl font-mono font-bold tracking-widest">{prizeCode.code}</p>
            </div>
            <p className="text-sm text-gray-400">El cajero validará este código y te entregará tu premio.</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white p-6 shadow-sm sticky top-0 z-10 flex justify-between items-center">
         ...
         <button onClick={() => setView('WELCOME')} className="text-xs text-red-400 border border-red-100 px-3 py-1 rounded-full">Salir</button>
      </div>
      ...
      <div className="p-6">
        {activeTab === 'checkin' && !scanning && (
           <div className="flex flex-col gap-6">
             <div className="w-full space-y-4">
               {user.memberships && user.memberships.length > 0 ? (
                 user.memberships.map((m: any, idx: number) => {
                   ...
                   const prizeName = m.tenant?.prize || "Premio Sorpresa"; // 👈 Nombre del premio
                     ...
                     <div key={idx} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 relative overflow-hidden">
                       ...
                       {!isWinner ? (
                         <>
                           ...
                           <div className="flex justify-between text-xs text-gray-400 font-bold uppercase"><span>0</span><span>Meta: 100 ({prizeName})</span></div>
                         </>
                       ) : (
                         <div className="animate-pulse">
                           <button 
                             onClick={() => getPrizeCode(m.tenantId, m.name)}
                             className="w-full bg-gradient-to-r from-yellow-400 to-orange-500 text-white font-bold py-3 rounded-xl shadow-md transform hover:scale-105 transition-all"
                           >
                             🏆 CANJEAR: {prizeName}
                           </button>
                         </div>
             ...
             <button onClick={() => setScanning(true)} className="w-full bg-black text-white py-5 rounded-2xl font-bold shadow-lg flex items-center justify-center gap-2">📷 Escanear QR</button>
           </div>
        )}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t flex justify-around p-2 pb-6">
        <button onClick={() => setActiveTab('checkin')} className="flex flex-col items-center p-2 text-blue-600">📍<span className="text-xs font-bold">Puntos</span></button>
      </div>
    </div>
  );
}

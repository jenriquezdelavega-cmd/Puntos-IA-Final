...
const Onboarding = () => {
  ...
  useEffect(() => {
    const i = setInterval(() => setSlide((p) => (p + 1) % 4), 4000);
        ...
        {slides.map((_, i) => (
          <div
            ...
            className={`h-1.5 w-1.5 rounded-full transition-all cursor-pointer ${
              i === slide ? 'bg-white w-4' : 'bg-white/40'
            }`}
          />
        ))}
      </div>
...
export default function Home() {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-orange-400 via-pink-500 to-purple-600 flex flex-col items-center justify-center p-6 text-white relative overflow-y-auto">
        <div className="w-full max-w-sm flex flex-col items-center py-10">
          ...
          {pendingCode && (
            <div className="bg-white/20 p-4 rounded-2xl mb-4 border border-white/30 backdrop-blur-sm animate-bounce w-full text-center">
          ...
          <div className="space-y-4 w-full mb-12">
            <button
              ...
              className="w-full bg-white text-pink-600 py-4 rounded-2xl font-extrabold text-lg shadow-xl hover:bg-gray-50 active:scale-95 transition-all"
            >
              Iniciar Sesión
            </button>
            ...
            <button
              ...
              className="w-full bg-white/10 border-2 border-white/50 text-white py-4 rounded-2xl font-bold text-lg hover:bg-white/20 active:scale-95 transition-all backdrop-blur-sm"
            >
              Crear Cuenta
            </button>
          </div>
      ...
      <div className="min-h-screen bg-gray-50 flex flex-col">
        <div className="bg-gradient-to-r from-orange-400 via-pink-500 to-purple-600 p-8 pb-20 pt-16 rounded-b-[3rem] shadow-xl text-white text-center relative z-0">
          <button
            ...
            className="absolute top-12 left-6 text-white/80 hover:text-white font-bold text-2xl transition-colors"
          >
            ←
          </button>
          <h2 className="text-3xl font-extrabold mt-2 tracking-tight">
            {isReg ? 'Únete al Club' : 'Bienvenido'}
          </h2>
          <p className="text-white/90 text-sm mt-1 font-medium">
            {isReg ? 'Premiamos tu lealtad, fácil y YA.' : 'Tus premios te esperan'}
          </p>
        </div>
          ...
          <div className="bg-white rounded-3xl shadow-2xl p-8 space-y-6 border border-gray-100">
            {isReg && (
              <div>
                ...
                <input
                  className="w-full p-4 bg-gray-50 rounded-2xl text-gray-800 font-bold border border-gray-100 focus:bg-white focus:ring-2 focus:ring-pink-400 outline-none transition-all"
                  value={name}
            ...
            {isReg && (
              <>
                <div>
                  ...
                  <input
                    type="email"
                    className="w-full p-4 bg-gray-50 rounded-2xl text-gray-800 font-medium border border-gray-100 focus:bg-white focus:ring-2 focus:ring-pink-400 outline-none transition-all"
                    value={email}
                ...
                <div className="flex gap-4">
                  <div className="flex-1">
                    ...
                    <input
                      type="date"
                      className="w-full p-4 bg-gray-50 rounded-2xl text-gray-800 font-medium border border-gray-100 h-[58px]"
                      value={birthDate}
            ...
            {message && (
              <div className="p-4 bg-red-50 text-red-500 rounded-2xl text-center font-bold text-sm border border-red-100">
                {message}
              </div>
            )}
            ...
            <button
              ...
              disabled={loading}
              className="w-full bg-gradient-to-r from-orange-500 to-pink-500 text-white py-4 rounded-2xl font-bold shadow-xl hover:shadow-2xl active:scale-95 transition-all text-lg mt-4"
            >
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-32">
      {showTutorial && (
        <div className="fixed inset-0 bg-gradient-to-br from-orange-400 via-pink-500 to-purple-600 z-[60] flex flex-col items-center justify-center p-8">
          <div className="w-full max-w-sm">
            <h2 className="text-white text-center font-black text-3xl mb-10">¿Cómo usar PuntoIA?</h2>
            <Onboarding />
            <button
              onClick={() => setShowTutorial(false)}
              className="w-full bg-white text-purple-600 font-bold py-4 rounded-2xl mt-12 shadow-xl hover:bg-gray-100"
            >
              ¡Entendido!
            </button>
          </div>
      ...
      {showHistory && (
        <div className="fixed inset-0 bg-black/90 z-[60] flex flex-col items-center justify-center p-6">
          <div className="bg-white p-6 rounded-[2rem] w-full max-w-md h-[70vh] flex flex-col shadow-2xl relative">
            <button
              onClick={() => setShowHistory(false)}
              className="absolute top-4 right-4 text-gray-400 font-bold p-2 text-xl hover:text-gray-600"
            >
              ✕
            </button>
      ...
      <div className="bg-white px-8 pt-16 pb-6 sticky top-0 z-20 shadow-sm flex justify-between items-center">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="h-12 w-12 bg-red-50 text-red-500 rounded-full font-bold border border-red-100 flex items-center justify-center hover:bg-red-500 hover:text-white transition-all shadow-sm"
          >
            ✕
          </button>
        </div>
        ...
        {activeTab === 'checkin' && !scanning && (
          <div className="flex flex-col gap-6">
              ...
              {user.memberships?.map((m: any, idx: number) => {
                ...
                return (
                  <div
                    ...
                    className={`bg-white p-6 rounded-[2rem] relative overflow-hidden transition-all duration-500 cursor-pointer border border-gray-100 ${
                      isExpanded ? 'shadow-2xl ring-4 ring-pink-50 scale-[1.02]' : 'shadow-lg hover:shadow-xl hover:scale-[1.01]'
                    }`}
                  >
                    ...
                    <div className="relative z-10">
                      <div className="flex justify-between items-start mb-6">
                        <div className="flex items-center gap-4">
                          <div className="h-14 w-14 rounded-2xl bg-gradient-to-br from-gray-900 to-gray-700 text-white flex items-center justify-center font-black text-2xl shadow-lg">
                            {m.name.charAt(0)}
                          </div>
                          <div>
                            <h3 className="font-bold text-gray-800 text-xl tracking-tight leading-none">{m.name}</h3>
                          </div>
                        </div>
                      ...
                      {!isWinner ? (
                        <>
                          <div className="relative w-full h-5 bg-gray-100 rounded-full overflow-hidden mb-3 shadow-inner">
                            ...
                            <div
                              className="h-full rounded-full bg-gradient-to-r from-orange-400 via-pink-500 to-purple-600 transition-all duration-1000 relative"
                              style={{ width: `${progress}%` }}
                            >
                              <div className="absolute top-0 left-0 bottom-0 right-0 bg-gradient-to-r from-transparent via-white/30 to-transparent w-full h-full"></div>
                            </div>
                          </div>

                          <div className="flex justify-between items-center text-xs font-bold uppercase tracking-wide">
                            <span className="text-gray-400 flex items-center gap-1">
                              {isExpanded ? '🔽 Menos info' : '▶️ Ver opciones'}
                            </span>
                            <span className="text-purple-600 bg-purple-50 px-2 py-1 rounded-lg">🏆 Meta: {m.prize}</span>
                          </div>
                        </>
                      ) : (
                        <button
                          ...
                          className="w-full bg-gradient-to-r from-yellow-400 via-orange-500 to-red-500 text-white font-black py-5 rounded-2xl shadow-xl animate-pulse tracking-wide text-lg relative z-20 hover:scale-[1.02] transition-transform border-4 border-white/20"
                        >
                          🎁 CANJEAR PREMIO
                        </button>
                      )}
                      ...
                      <div
                        className={`grid grid-cols-2 gap-3 mt-4 overflow-hidden transition-all duration-500 ease-in-out ${
                      ...
                      >
                        <button
                          ...
                          className="bg-white border-2 border-blue-50 text-blue-600 py-4 rounded-2xl font-bold text-xs flex flex-col items-center hover:bg-blue-50 transition-colors shadow-sm group"
                        >
                        ...
                        {m.instagram ? (
                          <a
                            ...
                            className="bg-white border-2 border-pink-50 text-pink-600 py-4 rounded-2xl font-bold text-xs flex flex-col items-center hover:bg-pink-50 transition-colors no-underline shadow-sm group"
                          >
                            <span className="text-2xl mb-1 group-hover:scale-110 transition-transform">📸</span>
                            Instagram
                          </a>
                        ) : (
                          <div className="bg-gray-50 border-2 border-gray-100 text-gray-300 py-4 rounded-2xl font-bold text-xs flex flex-col items-center grayscale opacity-50">
            ...
            <button
              ...
              className="w-full bg-gray-900 text-white py-6 rounded-[2.5rem] font-bold shadow-2xl flex items-center justify-center gap-3 active:scale-95 transition-all text-xl hover:bg-black group"
            >
            ...
            <div className="mt-8 flex justify-center">
              <div className="flex bg-white p-2 rounded-full shadow-sm border border-gray-100 w-full max-w-xs">
                <input
                  className="flex-1 p-2 bg-transparent text-gray-800 font-bold text-center tracking-[0.2em] uppercase text-sm outline-none placeholder-gray-300"
                ...
                <button
                  ...
                  disabled={!manualCode}
                  className="bg-gray-200 text-gray-600 font-bold px-4 rounded-full disabled:opacity-50 hover:bg-gray-300 transition-all"
                >
                  OK
                </button>
              </div>
        ...
        {scanning && (
          <div className="fixed inset-0 bg-black z-50 flex flex-col">
            ...
            <button
              ...
              className="absolute bottom-12 left-8 right-8 bg-white/20 backdrop-blur-md text-white p-5 rounded-3xl font-bold border border-white/20 shadow-2xl"
            >
              Cancelar Escaneo
            </button>
          </div>
        ...
        {activeTab === 'profile' && (
          <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100">
            <div className="flex items-center gap-5 mb-10">
              <div className="h-20 w-20 bg-gradient-to-br from-orange-100 to-pink-100 rounded-[1.5rem] flex items-center justify-center text-4xl shadow-inner text-pink-500">
                👤
              </div>
              <div>
                <h2 className="text-2xl font-black text-gray-900">Mi Perfil</h2>
                <p className="text-sm text-gray-400 font-medium">Gestiona tu identidad</p>
              </div>
            </div>

            <div className="space-y-6">
              <div>
                <label className="text-xs font-bold text-gray-400 uppercase ml-1 block mb-2 tracking-wider">Nombre</label>
                <input
                  className="w-full p-5 bg-gray-50 rounded-2xl text-gray-800 font-bold border border-transparent focus:bg-white focus:border-pink-200 outline-none transition-all"
                  value={name}
                  onChange={(e) => setName(e.target.value)}
                />
              </div>
              ...
              <div>
                <label className="text-xs font-bold text-gray-400 uppercase ml-1 block mb-2 tracking-wider">Teléfono</label>
                <input
                  type="tel"
                  maxLength={10}
                  className="w-full p-5 bg-gray-50 rounded-2xl text-gray-800 font-bold border border-transparent focus:bg-white focus:border-pink-200 outline-none transition-all"
              ...
              <div>
                <label className="text-xs font-bold text-gray-400 uppercase ml-1 block mb-2 tracking-wider">Email</label>
                <input
                  type="email"
                  className="w-full p-5 bg-gray-50 rounded-2xl text-gray-800 font-bold border border-transparent focus:bg-white focus:border-pink-200 outline-none transition-all"
                  value={email}
              ...
              <div className="flex gap-4">
                <div className="flex-1">
                  <label className="text-xs font-bold text-gray-400 uppercase ml-1 block mb-2 tracking-wider">Fecha</label>
                  <input
                    type="date"
                    className="w-full p-5 bg-gray-50 rounded-2xl text-gray-800 font-bold"
                    value={birthDate}
                    onChange={(e) => setBirthDate(e.target.value)}
                  />
                </div>
                ...
                <div className="flex-1">
                  <label className="text-xs font-bold text-gray-400 uppercase ml-1 block mb-2 tracking-wider">Género</label>
                  <select
                    className="w-full p-5 bg-gray-50 rounded-2xl text-gray-800 font-bold appearance-none"
                    value={gender}
                    onChange={(e) => setGender(e.target.value)}
                  >
                    <option value="Hombre">M</option>
            ...
            <button
              onClick={loadHistory}
              className="w-full bg-yellow-400 text-yellow-900 p-5 rounded-2xl font-bold mt-8 shadow-lg active:scale-95 transition-all text-lg hover:bg-yellow-300 flex items-center justify-center gap-2"
            >
            ...
            <button
              ...
              className="w-full bg-gray-900 text-white p-5 rounded-2xl font-bold mt-4 shadow-xl active:scale-95 transition-all text-lg hover:bg-black"
            >
              Guardar Cambios 💾
            </button>

            {message && (
              <p className="text-center text-green-600 mt-6 font-bold bg-green-50 p-4 rounded-2xl border border-green-100">
                {message}
              </p>
            )}
          </div>

...
export default function NegociosPage() {
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#05050a] text-white">
      ...
      <section className="relative z-10 mx-auto w-full max-w-6xl px-6 py-8">
        <div className="rounded-[30px] border border-white/10 bg-white/[0.03] p-6 md:p-8">
          <p className="text-[11px] font-black uppercase tracking-[0.2em] text-white/45">Prueba de encaje</p>
          <h3 className="mt-2 text-2xl font-black md:text-3xl">Negocios donde Punto IA genera mayor tracción.</h3>
          <div className="mt-6 grid gap-3 md:grid-cols-3">
            {[
              { type: 'Consumo frecuente', value: 'Cafeterías, snacks, restaurantes casuales' },
              { type: 'Ticket medio recurrente', value: 'Belleza, barbería, wellness, fitness boutique' },
              { type: 'Retail de cercanía', value: 'Tiendas especializadas y comercios de colonia' },
            ].map((segment) => (
              <article key={segment.type} className="rounded-2xl border border-white/10 bg-black/25 p-5">
                <p className="text-xs font-black uppercase tracking-[0.18em] text-white/45">{segment.type}</p>
                <p className="mt-2 text-sm font-semibold leading-relaxed text-white/75">{segment.value}</p>
              </article>
            ))}
          </div>
        </div>

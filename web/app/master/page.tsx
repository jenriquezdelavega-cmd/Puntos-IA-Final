...
export default function MasterPage() {
  ...
  const loadTenants = async (userOverride?: string, passOverride?: string, coalitionOverride?: boolean) => {
    try {
      const { response, data } = await postMasterJson('/api/master/list-tenants', {
      ...
      if (!response.ok) {
        return {
          ok: false as const,
          error: String(data.error || 'No se pudo validar el acceso master'),
        };
      }
      ...
      return {
        ok: false as const,
        error: 'Error de conexión al validar credenciales master',
      };
    }
  };
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    if (!tenantsOk.ok || !challengesOk || !walletOk) {
      const authMessage = tenantsOk.ok
        ? 'No se pudo cargar el panel master'
        : tenantsOk.error;
      alert(authMessage);
      return;
    }

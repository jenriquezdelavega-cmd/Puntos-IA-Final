...
export async function syncGoogleLoyaltyObjectForCustomer(params: {
  ...
  const primaryClassId = getGoogleWalletClassIdForTenant(tenant.id);
  const fallbackClassId = getGoogleWalletClassId();
  const classCandidates = Array.from(new Set([primaryClassId, fallbackClassId].filter(Boolean)));
  if (!classCandidates.length) return { ok: false as const, reason: 'class_missing' };
  ...
  const buildLoyaltyObject = (classId: string) => ({
    id: objectId,
    classId,
    state: 'ACTIVE',
  ...
  for (const classId of classCandidates) {
    try {
      ...
      const result = await upsertGoogleLoyaltyObject(buildLoyaltyObject(classId));
      if (result.operation === 'failed') {
        lastError = new Error(`Google loyalty object sync failed for class ${classId} with status ${result.status}`);
        continue;
      }

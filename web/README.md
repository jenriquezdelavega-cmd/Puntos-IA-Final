...
## Recuperación rápida cuando `main` no corre (merge markers) + traer cambios de `review-my-code`

Si Vercel falla con errores tipo `<<<<<<< HEAD`, ejecuta esto en Replit (desde `/home/runner/workspace`):
...
Si además quieres pasar cosas específicas de `review-my-code` a `main`, usa:

```bash
# trae set por defecto de Wallet/Pass
bash web/scripts/promote_from_ref_to_main.sh origin main origin/codex/review-my-code
git push origin main
```
...
Luego en Vercel: `Redeploy` y validar `/admin`, `/pass`, `/api/wallet/apple`.

## Replit: solo copy/paste (paso por paso)

> Objetivo: dejar `main` limpio, subirlo y validar en Vercel sin merges manuales.

### Paso 1 — Ir al root correcto
```bash
cd /home/runner/workspace
pwd
git rev-parse --show-toplevel
...
### Paso 2 — Traer remoto y pararte en `main`
```bash
git fetch origin
git checkout main
git pull origin main
...
### Paso 3 — Reparar markers (`<<<<<<<`, `=======`, `>>>>>>>`)
```bash
bash web/scripts/repair_conflict_markers.sh origin main origin/codex/review-my-code
```

### Paso 4 — Si quieres traer Wallet/Pass desde `review-my-code`
```bash
bash web/scripts/promote_from_ref_to_main.sh origin main origin/codex/review-my-code
```

### Paso 5 — Verificar que no queden markers
```bash
rg -n "^(<<<<<<<|=======|>>>>>>>)" web
```
...
### Paso 8 — Vercel
1. Abre Vercel > Deployments.
2. Busca el deploy de `main`.
3. Si no corre solo, clic en **Redeploy**.
4. Valida: `/admin`, `/pass`, `/api/wallet/apple`.

## Replit emergencia (cuando sigue saliendo `<<<<<<< HEAD`)

### Opción A: script único (recomendada)

```bash
cd /home/runner/workspace
git fetch origin
bash web/scripts/fix_main_conflicts_now.sh origin main origin/codex/review-my-code
git push origin main
```

### Opción B: si ese script no existe en tu Replit
...
FILES=$(rg -l "^(<<<<<<<|=======|>>>>>>>)" web || true)
echo "$FILES"
git checkout origin/codex/review-my-code -- $FILES
rg -n "^(<<<<<<<|=======|>>>>>>>)" web

git add $FILES
git commit -m "fix: clean committed conflict markers"
git push origin main
```
...
## Apple Pass: error `Faltan env var(s)`

Si al abrir `/api/wallet/apple` te sale algo como `Falta env var: APPLE_PASS_TYPE_ID`, configura **todas** estas variables en Vercel (Environment Variables):
...
También soportamos aliases comunes:
- `APPLE_PASS_TYPE_IDENTIFIER` o `NEXT_PUBLIC_APPLE_PASS_TYPE_ID` (alias de `APPLE_PASS_TYPE_ID`)
- `NEXT_PUBLIC_BASE_URL` o `NEXT_PUBLIC_APP_URL` (alias de `PUBLIC_BASE_URL`)

Si sigue saliendo `Falta env var: APPLE_PASS_TYPE_ID`, normalmente significa que el deploy activo aún no trae los cambios más recientes de `main`. Haz `Redeploy` del último commit de `main` y vuelve a probar.

Después de guardarlas:

1. Haz **Redeploy** del proyecto en Vercel.
2. Prueba con redirección habilitada (`-L`) para seguir `puntoia.mx -> www.puntoia.mx`:

```bash
curl -L -i "https://www.puntoia.mx/api/wallet/apple?customerId=TU_CUSTOMER_ID&businessId=TU_BUSINESS_ID"
```

Si sigue fallando, el endpoint ahora te devuelve la lista completa de variables faltantes para corregirlas en una sola vuelta.
Si la respuesta JSON trae `deployment.vercelCommitSha` y no coincide con tu último commit en `main`, estás viendo un deploy viejo: redeploy manual y vuelve a probar.

Para confirmar que sí estás pegándole al código nuevo, usa `-i` y revisa el header:

```bash
curl -L -i "https://www.puntoia.mx/api/wallet/apple?customerId=TU_CUSTOMER_ID&businessId=TU_BUSINESS_ID"
```

Debe aparecer `x-punto-wallet-version`. Si no aparece, sigues en un deployment viejo.
